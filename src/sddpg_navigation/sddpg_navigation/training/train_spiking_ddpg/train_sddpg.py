...
import argparse
from sddpg_navigation.training.train_spiking_ddpg.sddpg_agent import AgentSpiking
from sddpg_navigation.environment import *
from sddpg_navigation.utility import *


def train_sddpg(run_name="SNN_R1", 
                exp_name="Rand_R1", 
    ...
    weights_dir = 'save_sddpg_weights'
    try:
    ...
    while True:
        ...
        for ita in range(ita_per_episode):
            ...
            if overall_steps % save_steps == 0:
                max_w, min_w, max_b, min_b, shape_w, shape_b = agent.save(os.path.join(base_dir, weights_dir),
