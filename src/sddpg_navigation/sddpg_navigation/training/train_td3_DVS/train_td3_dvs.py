...
from sddpg_navigation.training.train_td3_DVS.td3_dvs_agent import Agent
...
class EventSubscriber(Node):
    """Buffers the latest decoded event frame from /camera/events."""

    def __init__(self):
        super().__init__('td3_dvs_event_subscriber')
        self.bridge = CvBridge()
        self.robot_events = np.zeros((2, 64, 64), dtype=np.float32)
        self.create_subscription(Image, '/camera/events', self._cb, _SENSOR_QOS)
...
def train_td3(run_name="TD3_DVS_R1", exp_name="Rand_R1", episode_num=(100, 200, 300, 400),
    ...
    weights_dir = 'save_td3_dvs_weights'
    try:
    ...
    agent = Agent(state_num, action_num, state_num,
    ...
    while True:
        ...
        for ita in range(ita_per_episode):
            ...
            raw_action = agent.act(rescale_state, event_frame)
            decode_action = wheeled_network_2_robot_action_decoder(raw_action, linear_spd_max, linear_spd_min)
            ...
            agent.remember_seq(state, rescale_state, event_frame,
                                raw_action, reward,
            ...
            if len(agent.memory) > batch_size:
                ...
                tb_writer.add_scalar('TD3_DVS/actor_loss', actor_loss_value, overall_steps)
                tb_writer.add_scalar('TD3_DVS/critic_loss', critic_loss_value, overall_steps)

            ita_time_end = time.time()
            tb_writer.add_scalar('TD3_DVS/ita_time', ita_time_end - ita_time_start, overall_steps)
            tb_writer.add_scalar('TD3_DVS/action_epsilon', agent.epsilon, overall_steps)
            tb_writer.add_scalar('TD3_DVS/raw_left_wheel_output', raw_action[0], overall_steps)
            tb_writer.add_scalar('TD3_DVS/raw_right_wheel_output', raw_action[1], overall_steps)

            if overall_steps % save_steps == 0:
                agent.save(os.path.join(base_dir, weights_dir), overall_steps // save_steps, run_name)
            ...
            if done or ita == ita_per_episode - 1:
                ...
                tb_writer.add_scalar('TD3_DVS/avg_reward', episode_reward / (ita + 1), overall_steps)
                break
...
def main(args=None):
    ...
    if args.cuda == 0:
        USE_CUDA = False
    train_td3(use_cuda=USE_CUDA, start_env=args.start_env)
    rclpy.shutdown()
